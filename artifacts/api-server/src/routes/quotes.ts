...
router.get("/quotes", requireTenantAccess, async (req, res) => {
  try {
    const quotes = await db.select().from(quotesTable).where(eq(quotesTable.tenantId, tid(req))).orderBy(sql`${quotesTable.createdAt} desc`);
    res.json(quotes);
...
router.post("/quotes", requireTenantAccess, async (req, res) => {
  try {
    ...
    const q = await db.insert(quotesTable).values({ ...req.body, reference: ref, tenantId: tid(req) }).returning();
...
router.get("/quotes/:id", requireTenantAccess, async (req, res) => {
  try {
    const q = await db.select().from(quotesTable).where(and(eq(quotesTable.id, Number(req.params.id)), eq(quotesTable.tenantId, tid(req)))).limit(1);
...
router.patch("/quotes/:id", requireTenantAccess, async (req, res) => {
  try {
    const q = await db.update(quotesTable).set(req.body).where(and(eq(quotesTable.id, Number(req.params.id)), eq(quotesTable.tenantId, tid(req)))).returning();
    if (!q.length) { res.status(404).json({ error: "Not found" }); return; }
    res.json(q[0]);
  } catch (err) { req.log.error(err); res.status(500).json({ error: "Internal server error" }); }
});
...
router.delete("/quotes/:id", requireTenantAccess, async (req, res) => {
  try {
    await db.delete(quotesTable).where(and(eq(quotesTable.id, Number(req.params.id)), eq(quotesTable.tenantId, tid(req))));
    res.status(204).send();
  } catch (err) { req.log.error(err); res.status(500).json({ error: "Internal server error" }); }
});
...
router.post("/quotes/:id/convert-to-project", requireTenantAccess, async (req, res) => {
  try {
    const q = await db.select().from(quotesTable).where(and(eq(quotesTable.id, Number(req.params.id)), eq(quotesTable.tenantId, tid(req)))).limit(1);
    if (!q.length) { res.status(404).json({ error: "Not found" }); return; }
    const project = await db.insert(projectsTable).values({ tenantId: tid(req), quoteId: q[0].id, customerId: q[0].customerId ?? undefined, title: `Project from ${q[0].reference}` }).returning();
...
router.get("/quotes/:id/items", requireTenantAccess, async (req, res) => {
  try {
    const items = await db.select().from(quoteItemsTable).where(eq(quoteItemsTable.quoteId, Number(req.params.id))).orderBy(quoteItemsTable.sortOrder);
    res.json(items);
  } catch (err) { req.log.error(err); res.status(500).json({ error: "Internal server error" }); }
});
