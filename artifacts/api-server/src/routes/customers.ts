...
import { requireTenantAccess, requireAuth } from "../middlewares/auth";
...
router.get("/customers", requireTenantAccess, async (req, res) => {
  try {
    const customers = await db.select().from(customersTable).where(eq(customersTable.tenantId, tid(req))).orderBy(sql`${customersTable.createdAt} desc`);
    res.json(customers);
...
router.post("/customers", requireTenantAccess, async (req, res) => {
  try {
    const c = await db.insert(customersTable).values({ ...req.body, tenantId: tid(req) }).returning();
...
router.get("/customers/:id", requireTenantAccess, async (req, res) => {
  try {
    const c = await db.select().from(customersTable).where(and(eq(customersTable.id, Number(req.params.id)), eq(customersTable.tenantId, tid(req)))).limit(1);
...
router.patch("/customers/:id", requireTenantAccess, async (req, res) => {
  try {
    const c = await db.update(customersTable).set(req.body).where(and(eq(customersTable.id, Number(req.params.id)), eq(customersTable.tenantId, tid(req)))).returning();
    if (!c.length) { res.status(404).json({ error: "Not found" }); return; }
    res.json(c[0]);
  } catch (err) { req.log.error(err); res.status(500).json({ error: "Internal server error" }); }
});
...
router.delete("/customers/:id", requireTenantAccess, async (req, res) => {
  try {
    await db.delete(customersTable).where(and(eq(customersTable.id, Number(req.params.id)), eq(customersTable.tenantId, tid(req))));
    res.status(204).send();
  } catch (err) { req.log.error(err); res.status(500).json({ error: "Internal server error" }); }
});
...
router.get("/portal/messages", requireAuth, async (req, res) => {
  try {
    ...
    const msgs = await db.select().from(portalMessagesTable).where(eq(portalMessagesTable.customerId, customer[0].id)).orderBy(sql`${portalMessagesTable.createdAt} asc`);
    res.json(msgs);
  } catch (err) { req.log.error(err); res.status(500).json({ error: "Internal server error" }); }
});
...
router.post("/portal/messages", requireAuth, async (req, res) => {
  try {
    ...
    const customer = await db.select().from(customersTable).where(eq(customersTable.clerkId, clerkId)).limit(1);
    if (!customer.length) { res.status(404).json({ error: "Customer not found" }); return; }
    const msg = await db.insert(portalMessagesTable).values({ ...req.body, customerId: customer[0].id, tenantId: customer[0].tenantId, senderRole: "customer" }).returning();
