...
router.get("/public/:tenantSlug/site", async (req, res) => {
  try {
    ...
    res.json({ tenant, settings: settings ?? null, featuredServices: services.filter(s => s.featured), featuredReviews: reviews, recentCaseStudies: caseStudies, featuredBeforeAfter: beforeAfter, globalFaqs: faqs });
