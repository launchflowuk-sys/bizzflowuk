...
import {
  ...
  useListContactMessages, useDeleteContactMessage,
...
const checkCls = "h-4 w-4 rounded border-slate-300 accent-[var(--brand)] cursor-pointer";

/**
 * Bulk row selection + fan-out delete over a page's single-delete mutation. One shared code path so
 * every table gets identical select-all / delete-selected behaviour. Deletions run in parallel via
 * Promise.allSettled — one failure never aborts the rest, and the toast reports the split.
 */
function useBulkSelect(allIds: number[], deleteOne: (id: number) => Promise<unknown>, invalidate: () => void, noun: string) {
  ...
  const toggleAll = () => setSelected(prev => (prev.size === allIds.length ? new Set<number>() : new Set(allIds)));
  const clear = () => { setSelected(new Set()); setConfirming(false); };
  const run = async () => {
    setDeleting(true);
    const ids = [...selected];
    ...
    invalidate();
    if (failed) showToast(`Deleted ${ids.length - failed} of ${ids.length} — ${failed} failed`, "error");
    else showToast(`${ids.length} ${noun}${ids.length === 1 ? "" : "s"} deleted`);
    setDeleting(false);
    clear();
  };
  return { selected, toggle, toggleAll, clear, run, deleting, confirming, setConfirming, allSelected: allIds.length > 0 && selected.size === allIds.length };
}

/** Floating action bar shown while rows are selected: count → confirm → delete selected. */
function BulkBar({ bulk, noun }: { bulk: ReturnType<typeof useBulkSelect>; noun: string }) {
  if (bulk.selected.size === 0) return null;
  return (
    <div className="sticky bottom-4 z-30 mx-auto flex w-fit items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-2.5 shadow-xl">
      <span className="text-sm font-semibold text-slate-700">{bulk.selected.size} selected</span>
      {bulk.confirming ? (
        <>
          <button onClick={bulk.run} disabled={bulk.deleting} className="inline-flex h-9 items-center rounded-lg bg-red-600 px-4 text-xs font-semibold text-white hover:bg-red-500 disabled:opacity-50 transition-colors">
            {bulk.deleting ? "Deleting..." : `Yes, delete ${bulk.selected.size}`}
          </button>
          <button onClick={() => bulk.setConfirming(false)} className="inline-flex h-9 items-center rounded-lg border border-slate-300 px-4 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors">Cancel</button>
        </>
      ) : (
        <button onClick={() => bulk.setConfirming(true)} className="inline-flex h-9 items-center rounded-lg bg-red-600 px-4 text-xs font-semibold text-white hover:bg-red-500 transition-colors">Delete selected</button>
      )}
      <button onClick={bulk.clear} className="text-xs font-medium text-slate-500 hover:text-slate-700">Clear</button>
    </div>
  );
}
...
/**
 * The rail, in named groups.
 *
 * Thirty-two destinations in one flat list ran past the bottom of a laptop
 * screen and well past the bottom of a phone, so the things people use every
 * day sat below the fold next to the things they touch twice a year. Unlabelled
 * dividers implied groups without naming them, which tells a reader nothing
 * they could not already see.
 *
 * Two entries stay pinned and always visible — Flo and the dashboard itself.
 * The rest collapse. Only the group holding the current page opens by default,
 * and a person's own choices are remembered, so the rail is as short as they
 * want it rather than as long as the feature list.
 */
// `module` gates the item in the sidebar (see useActiveModules below); omitted
// on core screens, which every business type always has.
type NavItem = { path: string; label: string; icon: string; brandMark?: boolean; module?: string };
...
const NAV_GROUPS: { key: string; label: string; items: NavItem[] }[] = [
  {
    ...
    items: [
      ...
      { path: "/dashboard/quotes", label: "Quotes", module: "quotes", icon: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
      { path: "/dashboard/payment-links", label: "Payment Links", module: "payments", icon: "M3 10h18M7 15h1m4 0h1m-7 4h12a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" },
  ...
  },
  {
    ...
    items: [
      { path: "/dashboard/projects", label: "Jobs", module: "jobs", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" },
      { path: "/dashboard/schedule", label: "Schedule", module: "schedule", icon: "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" },
      { path: "/dashboard/customers", label: "Customers", icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" },
      { path: "/dashboard/properties", label: "Properties", module: "properties", icon: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" },
      { path: "/dashboard/certificates", label: "Certificates", module: "certificates", icon: "M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" },
      { path: "/dashboard/files", label: "Files", module: "files", icon: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
    ],
  },
  {
    key: "money",
    label: "Money",
    items: [
      { path: "/dashboard/invoices", label: "Invoices", module: "invoices", icon: "M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z" },
      { path: "/dashboard/cash-flow", label: "Cash Flow", module: "cashflow", icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" },
      { path: "/dashboard/expenses", label: "Expenses", module: "expenses", icon: "M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" },
  ...
  },
  {
    ...
    label: "Your website",
    items: [
      { path: "/dashboard/website", label: "Your website", module: "website", icon: "M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" },
      { path: "/dashboard/services", label: "Services", module: "website", icon: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z" },
      { path: "/dashboard/pricing", label: "Pricing", module: "website", icon: "M9 8h6m-5 0a3 3 0 110 6H9l3 3m-3-9l3-3M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" },
      { path: "/dashboard/areas", label: "Areas", module: "website", icon: "M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" },
      { path: "/dashboard/gallery", label: "Gallery", module: "website", icon: "M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" },
      { path: "/dashboard/case-studies", label: "Case Studies", module: "website", icon: "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" },
      { path: "/dashboard/reviews", label: "Reviews", module: "website", icon: "M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" },
      { path: "/dashboard/faqs", label: "FAQs", module: "website", icon: "M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" },
      { path: "/dashboard/blog", label: "Blog", module: "website", icon: "M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" },
    ],
  },
  {
    key: "tools",
    label: "Getting found",
    items: [
      { path: "/dashboard/automations", label: "Automations", module: "automations", icon: "M13 10V3L4 14h7v7l9-11h-7z" },
      { path: "/dashboard/booking-qr", label: "QR code", module: "bookingqr", icon: "M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" },
      { path: "/dashboard/visualiser", label: "Visualiser", module: "visualiser", icon: "M15 10l4.553-2.069A1 1 0 0121 8.82v6.36a1 1 0 01-1.447.893L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" },
      { path: "/dashboard/messages", label: "Messages", module: "teamchat", icon: "M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" },
    ],
  },
  {
    key: "account",
...
/** Flat, for the page-title lookup and anything else that wants one list. */
const NAV_ITEMS = [...NAV_PINNED, ...NAV_GROUPS.flatMap(g => g.items)];
...
/**
 * The active business's resolved module list, or null before /me has loaded.
 * Null means "show everything" — a menu that briefly shows every item beats
 * one that flashes empty on first paint (see docs/plans/modules-system.md).
 */
function useActiveModules(): string[] | null {
  const { data: me } = useGetMe();
  const m = me as any;
  const businesses = (m?.businesses || []) as Array<any>;
  const active = businesses.find(b => b.tenantId === m?.tenantId) || businesses[0];
  return active?.modules ?? null;
}
 ...
 */
/** Which group a path belongs to, or null for a pinned item. */
function groupKeyForPath(path: string): string | null {
  for (const g of NAV_GROUPS) {
    if (g.items.some(i => path === i.path || path.startsWith(i.path + "/"))) return g.key;
  }
  return null;
}

const NAV_OPEN_KEY = "bf.rail.open";
...
function SidebarContent({ currentPath, onNavClick }: { currentPath: string; onNavClick?: () => void }) {
   ...
   *
   * Starts with only the group holding the current page, so arriving anywhere
   * shows a short rail with the relevant section already expanded rather than
   * everything at once. After that it is the person's own choice, remembered
   * between visits — somebody who lives in Invoices should not re-open Money
   * every morning.
   *
   * localStorage can throw outright in a private window, and a rail that
   * cannot render is far worse than one that forgets, so every access is
   * guarded.
   */
  const [openGroups, setOpenGroups] = useState<Record<string, boolean>>(() => {
    const active = groupKeyForPath(currentPath);
    const fallback = active ? { [active]: true } : {};
    try {
      const raw = localStorage.getItem(NAV_OPEN_KEY);
      if (!raw) return fallback;
      const saved = JSON.parse(raw) as Record<string, boolean>;
      return active ? { ...saved, [active]: true } : saved;
    } catch {
      return fallback;
    }
  });

  // Navigating into a collapsed group opens it, so the current page is never
  // hidden behind a heading the user has to guess at.
  useEffect(() => {
    const active = groupKeyForPath(currentPath);
  ...
  const toggleGroup = (key: string) => {
    setOpenGroups(prev => {
      ...
      try { localStorage.setItem(NAV_OPEN_KEY, JSON.stringify(next)); } catch { /* not worth breaking the rail over */ }
      return next;
    });
  };
  ...
  // Core items (no `module` tag) always show; a null list (still loading)
  // shows everything so the rail never flashes empty on first paint.
  const itemVisible = (item: NavItem) => !item.module || !modules || modules.includes(item.module);
    ...
    <>
      ...
      <nav className="ws-rail-nav flex-1 px-4 py-5 overflow-y-auto">
        {(() => {
          ...
          return (
            <>
              {NAV_PINNED.map(navLink)}

              {NAV_GROUPS.map(group => {
                // A business that has switched every item in a group off
                // (e.g. Splendid, no website module) loses the group header
                // too — an empty section is worse than no section.
                const items = group.items.filter(itemVisible);
                if (!items.length) return null;
                const open = !!openGroups[group.key];
                // A collapsed group still says whether the page you are on is
                // inside it, so nothing ever goes missing without a trace.
                const holdsCurrent = items.some(i => isActive(i.path));
                return (
                  <div key={group.key} className="ws-nav-group" data-open={open ? "true" : "false"}>
                    <button
                      type="button"
                      className="ws-nav-grouphead"
                      aria-expanded={open}
                      data-current={holdsCurrent ? "true" : "false"}
                      onClick={() => toggleGroup(group.key)}
                    >
                      <span>{group.label}</span>
                      <svg className="ws-nav-chevron" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                      </svg>
                    </button>
                    {/* 0fr -> 1fr animates to the content's own height with no
                        measurement and no max-height that clips a long group. */}
                    <div className="ws-nav-sublist">
                      <div>{items.map(navLink)}</div>
                    </div>
                  </div>
                );
              })}
...
function QuotesPage() {
  ...
  const bulk = useBulkSelect(
    ((quotes as any[]) ?? []).map((q: any) => q.id),
    (id) => deleteQuote.mutateAsync({ id } as any),
    () => qc.invalidateQueries({ queryKey: getListQuotesQueryKey() }),
    "quote",
  );
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteQuote.mutateAsync({ id } as any);
      qc.invalidateQueries({ queryKey: getListQuotesQueryKey() });
          ...
          <div className="hidden md:block rounded-xl border border-slate-200 bg-white overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {!(quotes as any[])?.length ? <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">No quotes yet</td></tr> : (quotes as any[])?.map((q: any) => (
                    <tr key={q.id} className="border-b border-slate-50 hover:bg-slate-50">
                      <td className="px-4 py-3"><input type="checkbox" className={checkCls} checked={bulk.selected.has(q.id)} onChange={() => bulk.toggle(q.id)} /></td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className="flex items-center gap-2 justify-end">
                          <Link href={`/dashboard/quotes/${q.id}`} className="inline-flex items-center rounded-lg bg-slate-100 text-slate-700 px-3 py-1.5 text-xs font-semibold hover:bg-slate-200 transition-colors">View</Link>
                          {deleteId === q.id ? (
                            <>
                              <button onClick={() => handleDelete(q.id)} className="text-xs text-white bg-red-500 hover:bg-red-600 px-2 py-0.5 rounded">Confirm</button>
                              <button onClick={() => setDeleteId(null)} className="text-xs text-slate-500 hover:text-slate-700 px-1">Cancel</button>
                            </>
                          ) : (
                            <button onClick={() => setDeleteId(q.id)} className="inline-flex items-center rounded-lg bg-red-50 text-red-600 px-3 py-1.5 text-xs font-semibold hover:bg-red-100 transition-colors">Delete</button>
                          )}
                        </span>
                      </td>
                    </tr>
...
function EmailsPage() {
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteEmail.mutateAsync({ id } as any);
      qc.invalidateQueries({ queryKey: getListSentEmailsQueryKey() });
        ...
        <>
          ...
          <div className="hidden md:block rounded-xl border border-slate-200 bg-white overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-100 bg-slate-50 text-xs text-slate-500 uppercase tracking-wide">
                  <th className="px-4 py-3 w-10"><input type="checkbox" className={checkCls} checked={bulk.allSelected} onChange={bulk.toggleAll} title="Select all" /></th><th className="text-left px-4 py-3">Subject</th><th className="text-left px-4 py-3">To</th><th className="text-left px-4 py-3">Status</th><th className="text-left px-4 py-3">Attachments</th><th className="text-left px-4 py-3">Sent</th><th className="px-4 py-3" />
                </tr></thead>
                <tbody>
                  {!rows.length ? <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">No emails sent yet</td></tr> : rows.map((m: any) => (
                    <tr key={m.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        {deleteId === m.id ? (
                          <span className="flex items-center gap-1 justify-end">
                            <button onClick={() => handleDelete(m.id)} className="text-xs text-white bg-red-500 hover:bg-red-600 px-2 py-0.5 rounded">Confirm</button>
                            <button onClick={() => setDeleteId(null)} className="text-xs text-slate-500 hover:text-slate-700 px-1">Cancel</button>
                          </span>
                        ) : (
                          <button onClick={() => setDeleteId(m.id)} className="inline-flex items-center rounded-lg bg-red-50 text-red-600 px-3 py-1.5 text-xs font-semibold hover:bg-red-100 transition-colors">Delete</button>
                        )}
                      </td>
                    </tr>
                  ))}
...
function MessagesPage() {
  ...
  const bulk = useBulkSelect(
    rows.map((m: any) => m.id),
    (id) => deleteMessage.mutateAsync({ id } as any),
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteMessage.mutateAsync({ id } as any);
      qc.invalidateQueries({ queryKey: getListContactMessagesQueryKey() });
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      ...
      <div className="space-y-3">
        {isLoading ? <div className="text-center py-12 text-slate-400">Loading...</div> : rows.length === 0 ? (
          <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-slate-400">No messages yet</div>
        ) : rows.map((m: any) => (
          <div key={m.id} onClick={() => setSelected(m)} className="w-full cursor-pointer text-left rounded-xl border border-slate-200 bg-white p-4 sm:p-5 space-y-1.5 hover:border-[var(--brand)]/60 hover:shadow-sm transition-all">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div className="flex min-w-0 items-center gap-2.5">
                <input type="checkbox" className={checkCls} checked={bulk.selected.has(m.id)} onClick={e => e.stopPropagation()} onChange={() => bulk.toggle(m.id)} />
            ...
            <div className="flex items-center gap-3 text-xs text-slate-400 pt-0.5">
              ...
              <span className="ml-auto flex items-center gap-2">
                {deleteId === m.id ? (
                  ...
                  <button onClick={e => { e.stopPropagation(); setDeleteId(m.id); }} className="inline-flex items-center rounded-lg bg-red-50 text-red-600 px-3 py-1.5 text-xs font-semibold hover:bg-red-100 transition-colors">Delete</button>
                )}
                <span className="font-medium text-[var(--brand-ink)]">Open &amp; reply →</span>
              </span>
            </div>
          </div>
        ))}
      </div>
      <BulkBar bulk={bulk} noun="message" />
