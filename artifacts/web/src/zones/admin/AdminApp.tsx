  ...
  return (
    <aside className="w-48 flex-shrink-0 bg-slate-900 min-h-screen flex flex-col">
      ...
      <nav className="flex-1 p-3">
        {nav.map(n => (
          <Link key={n.path} href={n.path} className={`flex items-center px-3 py-2 rounded-md text-xs font-medium mb-0.5 transition-colors ${currentPath === n.path || (n.path !== '/admin' && currentPath.startsWith(n.path)) ? 'bg-brand-500 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>{n.label}</Link>
        ))}
      </nav>
      ...
      {isLoading ? <div className="text-center py-12 text-slate-400">Loading...</div> : (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            { label: 'Total Leads', value: s?.leads, color: 'text-brand-600', bg: 'bg-brand-50' },
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/tenants/new" className="inline-flex h-9 items-center rounded-md bg-brand-500 px-4 text-sm font-medium text-white hover:bg-brand-400">+ Add Tenant</Link>
      </div>
...
function NewTenantPage() {
  ...
  const f = (key: string, label: string, type = 'text', placeholder?: string) => (
    <div key={key}><label className="block text-sm font-medium text-slate-700 mb-1">{label}</label>
      <input type={type} placeholder={placeholder} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" value={(form as any)[key]} onChange={e => setForm({...form, [key]: e.target.value})}/></div>
  );
  return (
    <div className="p-6 max-w-xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/admin/tenants" className="text-sm text-slate-500 hover:text-brand-500">&larr; Tenants</Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white rounded-xl border border-slate-200 p-6">
        ...
        <button type="submit" disabled={createMutation.isPending} className="w-full inline-flex h-10 items-center justify-center rounded-md bg-brand-500 text-sm font-semibold text-white hover:bg-brand-400 disabled:opacity-50">
